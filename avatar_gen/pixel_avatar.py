...
class PixelAvatar(object):
    ...
    def _create_image(self, matrix, width, height, filetype, padding):
        ...
        image = Image.new("RGB", (width + (padding * 2),
                          height + (padding * 2)), self.bg_colour)
            ...
            for col, cell in enumerate(cols):
                if cell:
                    image_draw.rectangle((
                        padding + col * block_width,  # x1
                        padding + row * block_height,  # y1
                        padding + (col + 1) * block_width - 1,  # x2
                        padding + (row + 1) * block_height - 1  # y2
                    ), fill=self.fg_colour)
